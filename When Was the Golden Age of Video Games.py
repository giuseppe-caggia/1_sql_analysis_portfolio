...
get_ipython().run_cell_magic('sql', '', 'postgresql:///games\n\n-- Paste your query from the previous task; update it to add a count of games released in each year called num_games\n-- Update the query so that it only returns years that have more than four reviewed games\n\nSELECT DISTINCT year, ROUND(AVG(critic_score),2) as avg_critic_score, count (*) as num_games\nFROM game_sales\nINNER JOIN reviews \non game_sales.game = reviews.game\nGROUP BY year\nHAVING COUNT(*) > 4\nORDER BY avg_critic_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'postgresql:///games\n\n-- Select year, an average of user_score, and a count of games released in a given year, aliased and rounded\n-- Include only years with more than four reviewed games; group data by year\n-- Order data by avg_user_score, and limit to ten results\nSELECT DISTINCT (year), ROUND(AVG(user_score),2) as avg_user_score, count (*) as num_games\nFROM game_sales\nINNER JOIN reviews \non game_sales.game = reviews.game\nGROUP BY year\nHAVING COUNT(*) > 4\nORDER BY avg_user_score DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'postgresql:///games\n\n-- Select the year results that appear on both tables\nSELECT year\nFROM top_critic_years_more_than_four_games AS top_critic_years \nINNER JOIN top_user_years_more_than_four_games AS top_user_years\nUSING (year)\nORDER BY year ASC;')
